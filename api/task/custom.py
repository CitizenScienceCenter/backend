...
def get_task_submissions(id=None, limit=20, offset=0):
    ...
    if t and t.submissions.count() > 0:
        return ResponseHandler(200, '', body=[s.to_dict() for s in t.submissions.limit(limit, offset=offset)]).send()
...
def get_task_media(id=None, limit=20, offset=0):
    ...
    if t and t.media.count() > 0:
        return ResponseHandler(200, '', body=[s.to_dict() for s in t.media.limit(limit, offset=offset)]).send()
